...
if ( ! class_exists( 'WKMP_Product_Queries' ) ) {
	...
	class WKMP_Product_Queries {
		 ...
		 */
		public function wkmp_get_seller_products( $data = array(), $seller_id ) {
			$wpdb_obj = $this->wpdb;
			$sql      = $wpdb_obj->prepare( "SELECT ID, post_title, post_status FROM {$wpdb_obj->prefix}posts WHERE post_type = 'product' AND post_author = %d", esc_attr( $seller_id ) );
		 ...
		 */
		public function wkmp_get_seller_total_products( $data = array(), $seller_id ) {
			$wpdb_obj = $this->wpdb;
