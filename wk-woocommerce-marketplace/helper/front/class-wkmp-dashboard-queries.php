...
if ( ! class_exists( 'WKMP_Dashboard_Queries' ) ) {
	...
	class WKMP_Dashboard_Queries {
		...
		public function wkmp_get_total_products_count( $seller_id ) {
			$wpdb_obj = $this->wpdb;
			$count    = $wpdb_obj->get_var( $wpdb_obj->prepare( "SELECT COUNT(*) AS 'product_count' FROM {$wpdb_obj->prefix}posts as posts WHERE post_type='product' AND post_author=%d AND post_status = 'publish'", $seller_id ) );
		...
		public function wkmp_get_sale_stats( $seller_id ) {
			$wpdb_obj = $this->wpdb;
			$result   = $wpdb_obj->get_row( $wpdb_obj->prepare( "SELECT seller_total_ammount, paid_amount, total_refunded_amount FROM {$wpdb_obj->prefix}mpcommision WHERE seller_id=%d", $seller_id ) );
