...
if ( ! class_exists( 'WKMP_Form_Field_Builder' ) ) {
	...
	class WKMP_Form_Field_Builder {
		...
		public function checkbox( $field, $f_key ) {
			$this->required_description( $field );
			?>
			<input type="checkbox" name="<?php echo esc_attr( $f_key ); ?>" id="<?php echo esc_attr( $f_key ); ?>" value="1" class="regular-text <?php echo ! empty( $field['class'] ) ? esc_attr( $field['class'] ) : ''; ?>" <?php checked( $field['value'], 1, true ); ?> />
			</td>
			</tr>
			<?php
		}
