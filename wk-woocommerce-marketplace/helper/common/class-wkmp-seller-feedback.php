	...
	class WKMP_Seller_Feedback {
		...
		public function wkmp_save_seller_feedback( $data, $seller_id ) {
			...
			$wpdb_obj->insert(
				...
				array(
					'seller_id'      => (int) $data['mp_wk_seller'],
					'user_id'        => (int) $data['mp_wk_user'],
					'price_r'        => (int) $data['feed_price'],
					'value_r'        => (int) $data['feed_value'],
					'quality_r'      => (int) $data['feed_quality'],
					'nickname'       => sanitize_text_field( wp_unslash( $data['nickname'] ) ),
					'review_summary' => sanitize_text_field( wp_unslash( $data['feed_summary'] ) ),
					'review_desc'    => sanitize_text_field( wp_unslash( $data['feed_review'] ) ),
					'review_time'    => gmdate( 'Y-m-d H:i:s' ),
					'status'         => 0,
				),
				array( '%d', '%d', '%d', '%d', '%d', '%s', '%s', '%s', '%s', '%d' )
			);
		...
		public function wkmp_get_seller_feedbacks( $filter_data ) {
			...
			if ( isset( $filter_data['filter_name'] ) && $filter_data['filter_name'] ) {
				$sql .= $wpdb_obj->prepare( ' AND review_summary LIKE %s', '%' . esc_attr( $filter_data['filter_name'] ) . '%' );
			}
