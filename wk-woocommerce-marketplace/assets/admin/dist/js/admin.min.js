...
/*!****************************************!*\
  !*** ./assets/admin/build/js/admin.js ***!
  \****************************************/
...
wkJQ(document).ready(function () {
  ...
  wkJQ('.wp-list-table.sellerorders').on('click', '.admin-order-pay', function () {
    var confirm = window.confirm(wkmpObj.commonConfirmMsg);

    if (confirm) {
      var order_seller_id = wkJQ(this).data('id');
      var anchor_el = wkJQ(this);
      var parent_el_td = wkJQ(anchor_el).parent('td');
      wkmp_update_order_status(order_seller_id, parent_el_td);
    }
  });
  wkJQ('.wkmp-approve-for-seller').on('click', function () {
    var curr_obj = wkJQ(this);
    var seller_id = curr_obj.data('seller-id');
    wkJQ.ajax({
      type: 'POST',
  ...
  wkJQ('.seller-query-revert').on('click', function (evt) {
    ...
    var query_id = wkJQ(this).data('qid');
    var reply_message = wkJQ(this).prev('div').find('.admin_msg_to_seller').val();
  ...
  }).trigger('change'); // Performing order action on seller action.

  wkJQ('select.wkmp_seller_order_action').on('change', function () {
    var select_el = wkJQ(this);
    var action_data = wkJQ(select_el).val();

    if (action_data) {
      var confirm = window.confirm(wkmpObj.commonConfirmMsg);

      if (confirm) {
        var parent_el_td = wkJQ(select_el).parent('td');
