...
$invoice_nonce = filter_input( INPUT_GET, 'invoice_nonce', FILTER_SANITIZE_STRING );

if ( ! empty( $invoice_nonce ) && wp_verify_nonce( $invoice_nonce, 'generate_invoice' ) ) {
	$admin_order              = new WC_Order( $order_id );
	...
	<div class="mp-invoice-wrapper">
		...
		<h1><?php esc_html_e( 'Invoice Of Order', 'wk-marketplace' ); ?> #<?php echo esc_html( $order_id ); ?></h1>
