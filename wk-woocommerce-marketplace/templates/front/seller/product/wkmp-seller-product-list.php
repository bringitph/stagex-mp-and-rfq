			...
			<div class="wkmp-table-action-wrap">
				...
				<div class="wkmp-action-section right wkmp-text-right">
					<?php if ( $wkmp_min_order_enabled || $wkmp_product_qty_limit_enabled ) { ?>
						<a id="wkmp_product_misc_settings" class="wkmp-minimum-order settings" href="javascript:void(0);"><?php esc_html_e( 'Change Product Miscellaneous Settings', 'wk-marketplace' ); ?></a>
					<?php } ?>
					...
					<a href="<?php echo esc_url( get_permalink() . get_option( '_wkmp_add_product_endpoint', 'add-product' ) ); ?>" class="button add-product" title="<?php esc_attr_e( 'Add Product', 'wk-marketplace' ); ?>"><span class="dashicons dashicons-plus-alt"></span></a>
				</div>
			</div>
		</form>
					...
					<?php if ( $products ) { ?>
						<?php foreach ( $products as $key => $product ) { ?>
							<tr>
								...
								<td>
									<?php if ( 'publish' === strtolower( $product['status'] ) ) { ?>
								...
								<td>
									<a href="<?php echo esc_url( $product['edit'] ); ?>" class="button" style="padding:12px;"><span class="dashicons dashicons-edit"></span></a>
									<a onclick="return confirm('Are you sure?')" href="<?php echo esc_url( $product['delete'] ); ?>" class="button" style="padding:12px;"><span class="dashicons dashicons-trash"></span></a>
								</td>
							</tr>
							<?php
						}
			...
			<div class="modal-body">
				<form action="" method="post" enctype="multipart/form-data" id="wkmp-seller-min-order-amount-form">
					...
					if ( $wkmp_min_order_enabled ) {
						...
						<div id="wkmp-amount-error" class="text-danger"></div>
						<?php
					}
					...
					if ( $wkmp_product_qty_limit_enabled ) {
						...
						<div id="wkmp-max-qty-limit-error" class="text-danger"></div>
						<?php
					}
