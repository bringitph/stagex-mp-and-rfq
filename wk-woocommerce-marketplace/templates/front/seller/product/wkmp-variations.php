...
$variable_image = WKMP_PLUGIN_URL . 'assets/images/placeholder.png';
...
<div class="wkmp_marketplace_variation">
	<h3>
		<button type="button" data-var_id="<?php echo esc_attr( $var_id ); ?>" class="mp_attribute_remove btn btn-danger wkmp_var_btn" rel="<?php echo esc_attr( $var_id ); ?>"><?php esc_html_e( 'Remove', 'wk-marketplace' ); ?></button>
		<input type='hidden' value="<?php echo esc_attr( $var_id ); ?>" name="mp_attribute_variation_name[]" id="mp_attribute_variation_id_<?php echo esc_attr( $var_id ); ?>"/>
		...
		?>
		<input class="variation_menu_order" name="wkmp_variation_menu_order[<?php echo esc_attr( $y ); ?>]" value="0" type="hidden">
	</h3>
	...
	<table style="display: table;" cellpadding="0" cellspacing="0">
		...
		<tr>
			<td class="sku" colspan="2">
				...
				<input size="5" id="wkmp_variation_sku_<?php echo esc_attr( $y ); ?>" class="wkmp_variable_sku wkmp_product_input" name="wkmp_variable_sku[<?php echo esc_attr( $y ); ?>]" value="<?php echo isset( $variation_arr['_sku'] ) ? esc_attr( $variation_arr['_sku'] ) : ''; ?>" placeholder="<?php echo isset( $variation_arr['_sku'] ) ? esc_attr( $variation_arr['_sku'] ) : ''; ?>" type="text">
				...
				<table class="data_table" cellpadding="0" cellspacing="0" style="display:table;">
					...
					<tr class="mpshow_if_variation_manage_stock wkmp_stock_status" style=" display:<?php echo ( isset( $variation_arr['_manage_stock'] ) && 'yes' === $variation_arr['_manage_stock'] ) ? 'table-row' : 'none'; ?>">
						...
						<td>
							...
							<select name="wkmp_variable_backorders[<?php echo esc_attr( $y ); ?>]" style="width:100%;">
					...
					<tr class="virtual" style="display:<?php echo ( isset( $variation_arr['_virtual'] ) && 'yes' === $variation_arr['_virtual'] ) ? 'none' : 'table-row'; ?>">
						...
						<td style="display: table-cell;" class="dimensions_field mp_hide_if_variation_virtual">
							...
							<input id="product_length_<?php echo esc_attr( $var_id ); ?>" class="input-text wc_input_decimal wkmp_product_input" size="6" name="wkmp_variable_length[<?php echo esc_attr( $y ); ?>]" value="<?php echo isset( $variation_arr['_length'] ) ? esc_attr( $variation_arr['_length'] ) : ''; ?>" placeholder="0" type="text">
							<input class="input-text wc_input_decimal wkmp_product_input" size="6" name="wkmp_variable_width[<?php echo esc_attr( $y ); ?>]" value="<?php echo isset( $variation_arr['_width'] ) ? esc_attr( $variation_arr['_width'] ) : ''; ?>" placeholder="0" type="text">
							<input class="input-text wc_input_decimal last wkmp_product_input" size="6" name="wkmp_variable_height[<?php echo esc_attr( $y ); ?>]" value="<?php echo isset( $variation_arr['_height'] ) ? esc_attr( $variation_arr['_height'] ) : ''; ?>" placeholder="0" type="text">
						</td>
					</tr>

					<?php do_action( 'mp_edit_variable_product_field', $var_id ); ?>

					<tr class="downloadable mpshow_if_variation_wkmp_downloadable<?php echo esc_attr( $y ); ?>" style="display: <?php echo ( isset( $variation_arr['_downloadable'] ) && 'yes' === $variation_arr['_downloadable'] ) ? 'table-row' : 'none'; ?>">
						<td colspan="2">
							<div class="form-field downloadable_files">
								<label><?php esc_html_e( 'Downloadable Files:', 'wk-marketplace' ); ?></label>
								<div class="widefat">
									<div class="wkmp_variation_downloadable_file" id="variation_downloadable_file_<?php echo esc_attr( $y ); ?>">
										<?php
										...
										foreach ( $product_vars_downloadables as $pro_downloadable ) {
											...
											<div class="tr_div">
												<div>
													<label for="downloadable_upload_file_name_<?php echo esc_attr( $y . '_' . $i ); ?>"><?php esc_html_e( 'File Name', 'wk-marketplace' ); ?></label>
													<input type="text" class="input_text wkmp_product_input" placeholder="<?php esc_attr_e( 'File Name', 'wk-marketplace' ); ?>" id="downloadable_upload_file_name_<?php echo esc_attr( $y . '_' . $i ); ?>" name="_mp_variation_downloads_files_name[<?php echo esc_attr( $y ); ?>][<?php echo esc_attr( $i ); ?>]" value="<?php echo isset( $pro_downloadable['name'] ) ? esc_attr( $pro_downloadable['name'] ) : ''; ?>">
												</div>
												<div class="file_url">
													<label for="downloadable_upload_file_url_<?php echo esc_attr( $y . '_' . $i ); ?>"><?php esc_html_e( 'File Url', 'wk-marketplace' ); ?></label>
													<input type="text" class="input_text wkmp_product_input" placeholder="http://" id="downloadable_upload_file_url_<?php echo esc_attr( $y . '_' . $i ); ?>" name="_mp_variation_downloads_files_url[<?php echo esc_attr( $y ); ?>][<?php echo esc_attr( $i ); ?>]" value="<?php echo isset( $pro_downloadable['file'] ) ? esc_attr( $pro_downloadable['file'] ) : ''; ?>">
													<a href="javascript:void(0);" class="button wkmp_downloadable_upload_file" id="<?php echo esc_attr( $y . '_' . $i ); ?>"><?php esc_html_e( 'Choose&nbsp;file', 'wk-marketplace' ); ?></a>
													<a href="javascript:void(0);" class="delete mp_var_del" id="mp_var_del_<?php echo esc_attr( $y . '_' . $i ); ?>"><?php esc_html_e( 'Delete', 'wk-marketplace' ); ?></a>
												</div>
					...
					<tr class="downloadable" style="display:<?php echo ( isset( $variation_arr['_downloadable'] ) && 'yes' === $variation_arr['_downloadable'] ) ? 'table-row' : 'none'; ?>">
						<th colspan="4" class="wkmp_add_file_middle">
							<a href="javascript:void(0);" class="button mp_varnew_file mp_var_down_load_call_<?php echo esc_attr( $y ); ?>" id="<?php echo esc_attr( $y ); ?>"><?php esc_html_e( 'Add File', 'wk-marketplace' ); ?></a>
						</th>
					</tr>
					<?php
						...
						<tr class="downloadable mpshow_if_variation_wkmp_downloadable<?php echo esc_attr( $y ); ?>" style="display:<?php echo ( isset( $variation_arr['_downloadable'] ) && 'yes' === $variation_arr['_downloadable'] ) ? 'table-row' : 'none'; ?>">
							<td>
								<div>
									...
									<input size="5" name="wkmp_variable_download_limit[<?php echo esc_attr( $y ); ?>]" value="<?php echo isset( $variation_arr['_download_limit'] ) ? esc_attr( $variation_arr['_download_limit'] ) : ''; ?>" placeholder="<?php esc_attr_e( 'Unlimited', 'wk-marketplace' ); ?>" step="1" min="0" type="number" class="wkmp_product_input"/>
								</div>
							</td>
							<td>
								<div>
									<label><?php esc_html_e( 'Download Expiry:', 'wk-marketplace' ); ?></label>
									<input size="5" name="wkmp_variable_download_expiry[<?php echo esc_attr( $y ); ?>]" value="<?php echo isset( $variation_arr['_download_expiry'] ) ? esc_attr( $variation_arr['_download_expiry'] ) : ''; ?>" placeholder="<?php esc_attr_e( 'Unlimited', 'wk-marketplace' ); ?>" step="1" min="0" type="number" class="wkmp_product_input"/>
								</div>
							</td>
						</tr>
						<?php
		...
		<tr>
			<td class="wkmp_upload_image_variation">
				<a href="javascript:void(0);" class="upload_var_image_button" id="var_img_<?php echo esc_attr( $y ); ?>"><img src="<?php echo esc_url( $variable_image ); ?>" id="wkmp_variation_product_var_img_<?php echo esc_attr( $y ); ?>"><input name="upload_var_img[<?php echo esc_attr( $y ); ?>]" id="upload_var_img_<?php echo esc_attr( $y ); ?>" value="<?php echo esc_attr( $thumb_id ); ?>" type="hidden"></a>
			</td>
			<td class="options">
				<label>
					<input class="checkbox" name="wkmp_variable_enabled[<?php echo esc_attr( $y ); ?>]" type="checkbox" checked='checked'>
					<?php esc_html_e( 'Enabled', 'wk-marketplace' ); ?>
				</label>
				<label title="check/uncheck to sell downloadable products">
					<input class="checkbox checkbox_is_downloadable" id="wkmp_downloadable<?php echo esc_attr( $y ); ?>" value="yes" name="wkmp_variable_is_downloadable[<?php echo esc_attr( $y ); ?>]" type="checkbox" <?php echo ( isset( $variation_arr['_downloadable'] ) && 'yes' === $variation_arr['_downloadable'] ) ? 'checked' : ''; ?>>
					<?php esc_html_e( 'Downloadable', 'wk-marketplace' ); ?>
				</label>
				<label title="check/uncheck to assign its weight, dimensions">
					<input class="checkbox checkbox_is_virtual" id="wkmp_virtual<?php echo esc_attr( $y ); ?>" value="yes" name="wkmp_variable_is_virtual[<?php echo esc_attr( $y ); ?>]" type="checkbox" <?php echo ( isset( $variation_arr['_virtual'] ) && 'yes' === $variation_arr['_virtual'] ) ? 'checked' : ''; ?>>
					<?php esc_html_e( 'Virtual', 'wk-marketplace' ); ?>
				</label>
				<label title="check/uncheck to manage stock">
					<input class="checkbox checkbox_manage_stock" id="wkmp_stock<?php echo esc_attr( $y ); ?>" value="yes" name="wkmp_variable_manage_stock[<?php echo esc_attr( $y ); ?>]" type="checkbox" <?php echo ( isset( $variation_arr['_manage_stock'] ) && 'yes' === $variation_arr['_manage_stock'] ) ? 'checked' : ''; ?>>
