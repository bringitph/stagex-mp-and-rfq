	...
	<?php if ( ! empty( $order_data ) ) { ?>
		<div class="woocommerce-MyAccount-content mp-order-view wrap">
			<div id="order_data_details">
				...
				<h3><?php esc_html_e( 'Order', 'wk-marketplace' ) . ' #' . esc_html( $order_id ); ?></h3>
					...
					<form method="post" id="wkmp-order-view-form">
						<table class="widefat">
							...
							foreach ( $order_data as $product_id => $details ) {
								$total_payment = apply_filters( 'wkmp_add_order_fee_to_total', $mp_order_data['order_id'], round( floatval( $mp_order_data['total_seller_amount'] ), 2 ) );
									...
									<tr class="order_item alt-table-row">
										...
										<td class="product-refund toptable wkmp-order-refund" style="display:none;">
											...
											if ( ! empty( $seller_order_refund_data['line_items'][ $details['item_key'] ] ) && $seller_order_refund_data['line_items'][ $details['item_key'] ]['qty'] >= $product_qty ) {
												...
												$product_amount = ( $details[0]['product_total_price'] - $mp_order_data['product'][ $product_id ]['commission'] ) / $product_qty;
												?>
							...
							if ( $mp_order_data['product_total'] !== $mp_order_data['total_seller_amount'] ) {
								...
								$tip = apply_filters( 'wkmp_rfq_add_order_fee_to_tooltip', $mp_order_data['order_id'], $tip );
							...
							foreach ( $seller_order->get_items( 'shipping' ) as $item_id => $shipping_item_obj ) {
								$method_id      = $wpdb_obj->get_var( $wpdb_obj->prepare( "SELECT meta_value FROM {$wpdb_obj->prefix}mporders_meta WHERE meta_key='shipping_method_id' AND order_id=%d AND seller_id=%d", $order_id, $seller_id ) );
								$order_ship_ids = array( $shipping_item_obj->get_method_id(), $shipping_item_obj->get_method_id() . $shipping_item_obj->get_instance_id() );

								if ( ! in_array( $method_id, $order_ship_ids, true ) ) {
									continue;
								}
							...
							do_action( 'wkmp_add_fee_to_seller_order_view', $mp_order_data['order_id'], $cur_symbol );
							if ( ! empty( $mp_order_data['tax'] ) ) {
								?>
								<tr class="alt-table-row">
							...
							<tr class="alt-table-row">
								...
								<td class="toptable" colspan="2">
									<?php if ( ! empty( $seller_order_refund_data['refunded_amount'] ) ) { ?>
										...
										<?php } ?>
										<span class="amount"> <?php echo wp_kses_data( wc_price( $total_payment - apply_filters( 'wkmp_add_order_fee_to_total', $mp_order_data['order_id'], round( floatval( $seller_order_refund_data['refunded_amount'] ), 2 ) ), array( 'currency' => $order_currency ) ) ); ?></span>
									<?php } else { ?>
							...
							if ( ! empty( $seller_order_refund_data['refunded_amount'] ) ) {
								...
								<tr class="alt-table-row wkmp-green">
									...
									<td class="toptable" colspan="3">
										<p class="amount"><?php echo wp_kses_data( wc_price( apply_filters( 'wkmp_add_order_fee_to_total', $mp_order_data['order_id'], round( floatval( $seller_order_refund_data['refunded_amount'] ), 2 ) ), array( 'currency' => $order_currency ) ) ); ?></p>
									</td>
								</tr>
							<?php } ?>
			...
			<div class="mp-status-manage-class">
				...
				// JS edit: Purchased button on Sellers order view. 3/8
				...
				// if ( 'wc-completed' !== $order_status ) {
					?>
					<!-- JS edit: Add warning on Purchased button -->
					<form method="POST" onSubmit="return confirm('Important: By pressing OK, you confirm that you have purchased the item according to your Buyer\'s exact specifications and subsequent comments. \n\nPlease send your Buyer a copy of the official store receipt through the Comments section of the order. ') ">
						<table class="shop_table shop_table_responsive customer_details widefat">
							<tbody>
							<tr>
								<!-- JS edit: Purchased button on Sellers order view. 4/8 -->
								...
								// JS edit: Purchased button on Sellers order view. 5/8
								</td> -->	
								<td>
								...
								<!-- JS edit: Purchased button on Sellers order view. 6/8 -->
								<td></td>
							</tr>
							
							<!-- JS edit: Purchased button on Sellers order view. 7/8 -->
							</tbody>
					...
					// JS edit: Purchased button on Sellers order view. 8/8
					</form> -->
				<?php //} else { ?>
