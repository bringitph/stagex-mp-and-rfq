...
if ( ! class_exists( 'WKMP_Install' ) ) {
	...
	class WKMP_Install {
		...
		public function wkmp_create_schema() {
			...
			$this->wkmp_migrate_settings();
			$this->wkmp_migrate_email_settings();
		}
		...
		public function wkmp_create_seller_role() {
			global $wp_roles, $wkmarketplace;

			$role = 'wk_marketplace_seller';
			if ( get_role( $role ) ) {
				$wkmarketplace->log( "Role: $role is already created." );
		...
		public function wkmp_create_db_tables() {
			...
			$wkmarketplace->log( 'All tables are successfully created.' );
		}
		 ...
		 * @since 5.0.1
		 */
		public function wkmp_migrate_settings() {
		...
		/**
		 * Migrate Email settings if updating from 5.1.0
		 *
		 * @since 5.1.2
		 */
		public function wkmp_migrate_email_settings() {
			$option_keys = array(
			...
			update_option( '_wkmp_db_version', WKMP_DB_VERSION, true );
		}
	}
}
