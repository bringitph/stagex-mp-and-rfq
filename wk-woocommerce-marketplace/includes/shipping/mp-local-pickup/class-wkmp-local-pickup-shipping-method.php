		...
		public function calculate_shipping( $package = array() ) {
			...
			if ( 'yes' === $this->enabled ) {
				foreach ( $package['contents'] as $values ) {
					...
					if ( $seller_id > 0 ) {
						$seller_zones     = $this->get_zone_id_from_package( $package, $seller_ids );
						$zone             = new WC_Shipping_Zone( $seller_zones ? $seller_zones : 0 );
						$shipping_methods = $zone->get_shipping_methods( true );

						foreach ( $shipping_methods as $sm_value ) {
							if ( 'mp_local_pickup' === $sm_value->id ) {
								$method = true;
							}
						}
					}
						...
						if ( isset( $seller_zones ) ) {
							...
							if ( ! empty( $methods ) && isset( $methods ) ) {
								foreach ( $methods as $shipping_method ) {
									if ( 'mp_local_pickup' === $shipping_method->id && ! in_array( $seller_details, $ids, true ) ) {
