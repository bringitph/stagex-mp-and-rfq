...
$seller_order_refund_data = $order_refund->wkmp_get_seller_order_refund_data( $seller_order_id );
...
if ( ! empty( $refunded_amount ) ) {
	$refund_statement = '<p>' . html_entity_decode( /* translators: %1$s Order number, %2$s Price. */ sprintf( esc_html__( 'Your order %1$s has been refunded by amount %2$s. There are more details below for your reference:', 'wk-marketplace' ), esc_html( $seller_order->get_order_number() ), wc_price( $refunded_amount, array( 'currency' => $seller_order->get_currency() ) ) ), ENT_QUOTES, 'UTF-8' ) . '</p>';
} else {
	$refund_statement = '<p>' . html_entity_decode( /* translators: %s Order number. */ sprintf( esc_html__( 'Your order %s has been fully refunded. There are more details below for your reference:', 'wk-marketplace' ), esc_html( $seller_order->get_order_number() ) ), ENT_QUOTES, 'UTF-8' ) . '</p>';
}
...
do_action( 'woocommerce_email_footer', $detail_count );
