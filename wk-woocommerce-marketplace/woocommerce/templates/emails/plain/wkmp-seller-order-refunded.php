...
$seller_order             = is_array( $data ) ? new WC_Order( $data[0]->get_order_id() ) : new WC_Order( $data->get_order_id() );
...
$seller_order_refund_data = $order_refund->wkmp_get_seller_order_refund_data( $seller_order->get_id() );
...
$result  = utf8_decode( esc_html__( 'Your ', 'wk-marketplace' ) ) . '&nbsp; Order #' . $seller_order->get_ID() . ' (' . $date_string . ') ' . "\n\n";
$result .= utf8_decode( esc_html__( ' has been refunded by amount ', 'wk-marketplace' ) ) . '&nbsp; ' . wc_price( $refunded_amount, array( 'currency' => $seller_order->get_currency() ) ) . "\n\n";
