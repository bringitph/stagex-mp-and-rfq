...
$result = '
	<div style="margin-bottom: 40px;">
		<p>' . html_entity_decode( sprintf( /* translators: %1$s Order number. */ esc_html__( 'Just to let you know, your order #%s status is now being changed to processing:', 'wk-marketplace' ), esc_html( $seller_order->get_order_number() ) ), ENT_QUOTES, 'UTF-8' ) . '</p>
	...
	for ( $i = 0; $i < $detail_count; ++ $i ) {
		...
		if ( 0 === intval( $details[ $i ]['variable_id'] ) ) {
			...
			$result .= '</td><td class="td">' . $details[ $i ]['qty'] . '</td>
									...
									' . html_entity_decode( wc_price( $details[ $i ]['product_total_price'], array( 'currency' => $seller_order->get_currency() ) ), ENT_QUOTES, 'UTF-8' ) . '
								</td>
							</tr>';
		} else {
